...
class Widget_Users_Profile extends Widget_Users_Edit implements Widget_Interface_Do
{
    ...
    public function profileForm()
    {
        ...
        /** 用户昵称 */
        $screenName = new Typecho_Widget_Helper_Form_Element_Text('screenName', NULL, NULL, _t('昵称'), _t('昵称默认与账号相同，昵称用于前台显示，账号用于登录.'));
        ...
        /** 电子邮箱地址 */
        $mail = new Typecho_Widget_Helper_Form_Element_Text('mail', NULL, NULL, _t('校园邮箱地址 （重要）*'), _t('邮箱地址作为您的身份认证和账号找回依据.')
            . '<br />' . _t('请使用您的校园邮箱（身份认证仅支持@mail.sysu.edu.cn和@mail2.sysu.edu.cn后缀邮箱）.'));
        $form->addInput($mail);
        ...
        /** 提交按钮 */
        $submit = new Typecho_Widget_Helper_Form_Element_Submit('submit', NULL, _t('更新个人信息'));
        $submit->input->setAttribute('class', 'btn btn-success');
        $form->addItem($submit);
...
public function personalForm($pluginName, $className, $pluginFileName, &$group)
{
	...
	$submit = new Typecho_Widget_Helper_Form_Element_Submit(NULL, NULL, _t('更新状态'));
	$submit->input->setAttribute('class', 'btn btn-success');
    $form->addItem($submit);
    ...
    public function passwordForm()
    {
        ...
        /** 用户密码 */
        $password = new Typecho_Widget_Helper_Form_Element_Password('password', NULL, NULL, _t('用户密码'), _t('请输入一个新的密码.')
            . '<br />' . _t('建议使用特殊字符与字母、数字的混编样式,以增加账户安全性.'));
     ...
     * @return boolean
     */
    public function changevstatus()
    {
        $a = 1;
        $this->update(array('vstatus' => $a),
        $this->db->sql()->where('uid = ?', $this->user->uid));
    }
