 ...
 *
 */
class Userv_Plugin implements Typecho_Plugin_Interface
{
...
//		Typecho_Plugin::factory('Widget_Archive')->select = array('UserSign_Plugin', 'selectHandle');
	...
	public static function sqlInstall()
	{
		...
		$prefix = $db->getPrefix();
			...
			$select = $db->select('table.users.vstatus')->from('table.users');
			$db->query($select);
			return '检测到验证字段，插件启用成功';
		 /*catch (Typecho_Db_Exception $e) {
			...
			throw new Typecho_Plugin_Exception('数据表检测失败，插件启用失败。);
		}*/
	}
    ...
    public static function personalConfig(Typecho_Widget_Helper_Form $form){
		...
		$sign = new Typecho_Widget_Helper_Form_Element_Text('vstatus', NULL, $res['vstatus'], _t('helloworld'));
		$form->addInput($sign);
	}
	...
	public static function personalConfigHandle($settings,$isSetup){
		...
		$db->query($db->sql()->where('uid = ?', $user->uid)->update('table.users')->rows(array('vstatus'=>Typecho_Common::removeXSS($settings['vstatus']))));
	}
